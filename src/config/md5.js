...
export const md5 = function(){
    ...
    function binl2b64(d) {
        for (var r = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", _ = "", m = 0; m < 4 * d.length; m += 3) for (var n = (d[m >> 2] >> 8 * (m % 4) & 255) << 16 | (d[m + 1 >> 2] >> 8 * ((m + 1) % 4) & 255) << 8 | d[m + 2 >> 2] >> 8 * ((m + 2) % 4) & 255, h = 0; h < 4; h++) _ += 8 * m + 6 * h > 32 * d.length ? b64pad : r.charAt(n >> 6 * (3 - h) & 63);
        return _
    }
